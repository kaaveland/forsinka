...
pub fn prepare_db(
    ...
    db.execute("create or replace table quays as from read_parquet($1);", [quays.as_str()])?;
    db.execute("create or replace table stops as from read_parquet($1);", [stops.as_str()])?;
